...
class Box():
    ...
    def get_metedata_mp4(self, video_name, down_path):
        ...
        if self.Version == 0:
            ...
            with open('data/yt_crawled_url/ERROR.log', 'a', encoding='utf-8') as f:
                f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S  ") + username +video_name + ', ' + str(self.itag) )
            print("version wrong")
            self.Earliest_Presentation_Time = int.from_bytes(sidx[:4], byteorder='big')
            sidx = sidx[4:]
            self.First_Offset = int.from_bytes(sidx[:4], byteorder='big')
    ...
    def get_metedata_webm(self, video_name, down_path):
        ...
        if os.path.exists(videopath):
            ...
            with open('data/yt_crawled_url/ERROR.log', 'a', encoding='utf-8') as f:
                f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S  ") + username + ' ' + video_name + ', ' + str(self.itag) + ', invalid video.\n') # 未下载该视频
                return
...
class Video():
    ...
    def analyse_websource(self):
        ...
        for param in service_tracking_params:
            itag = param.get('itag')
            if itag in self.itag_list: # 重复itag，这里以第一次出现为准
                print(f'{itag} once more.....')
                continue
            if itag in (self.video_mp4_itag + self.video_webm_itag):
                ...
                if indexRange == {'start':0, 'end':0}:
                    with open('data/yt_crawled_url/ERROR.log', 'a', encoding='utf-8') as f:
                        f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S  ") + username + ' ' + self.url+', '+str(itag)+ ', invalid indexRange.\n') # 该itag没有indexRange
                else:
                ...
                with open('data/yt_crawled_url/newitag.log', 'a', encoding='utf-8') as f:
                    f.write(self.url + ',newitag:' + str(itag)+ '\n')
            ...
            for itag in self.itag_list:
                ...
                if hasattr(box, 'reference_list'):
                    if np.any(np.array(box.duration_list) < 0):
                        with open('data/yt_crawled_url/ERROR.log', 'a', encoding='utf-8') as f:
                                f.write(datetime.now().strftime("%Y-%m-%d %H:%M:%S  ") + username + ' ' + self.url+', '+str(itag)+ ', nagtive value in fp.\n')
                        continue
...
################################################
global video_list, process_count, sleep_time, username
# 'data/yt_crawled_url/dogchinashow_url_20240609014807.csv', 'data/yt_crawled_url/gongzishen_url_20240609014706.csv', 'data/yt_crawled_url/Taiwan16888_url_20240609014604.csv', 'data/yt_crawled_url/TianLiangTimes_url_20240609014413.csv', 'data/yt_crawled_url/user-jt1zr1ey7v_url_20240609014503.csv',  'data/yt_crawled_url/user-tp1lk6kj1g_url_20240609015114.csv', 
# 'data/yt_crawled_url/user-xl8fu9dw5q_url_20240609013824.csv', 'data/yt_crawled_url/WuJianMin_url_20240609014908.csv', 
